...
def new_method_proxy(func):
    """
    Create a proxy method that delegates to the wrapped object.

    The proxy checks if the wrapped object is initialized, calling _setup()
    if needed, then delegates the operation to the wrapped object.
    """
    ...
    # Mark this as a proxy method so __getattribute__ can detect it
...
class SimpleLazyObject(LazyObject):
    ...
    def __init__(self, func):
        ...
        in various circumstances, then you must ensure that the callable can be
        safely run more than once and will return the same value.
        """
...
# =============================================================================
# Lazy Function Evaluation (for deferred computation)
# =============================================================================
...
def lazy(func, *resultclasses):
    ...
    are required -- at least one is needed so that the automatic forcing of
    the lazy evaluation code is triggered. Results are not memoized; the
    function is evaluated on every access.

    Usage:
        # Create a lazy string
        lazy_str = lazy(lambda: expensive_computation(), str)()

        # For translation
        from nitro.utils.translation import gettext
        gettext_lazy = lazy(gettext, str)
        msg = gettext_lazy("Hello")  # Translation happens when msg is used

    Args:
        func: The callable to be evaluated lazily
        *resultclasses: One or more classes that the result will be an instance of

    Returns:
        A wrapper function that returns a Promise proxy object
    """
...
def _lazy_proxy_unpickle(func, args, kwargs, *resultclasses):
    """Helper function to unpickle lazy proxy objects."""
...
def keep_lazy(*resultclasses):
    ...
    This is useful for utility functions that should preserve laziness.
    ...
    Usage:
        ...
        # Now works with both regular and lazy strings
        regular = upper_case('hello')  # Returns 'HELLO' immediately
        lazy_msg = gettext_lazy('hello')
        lazy_upper = upper_case(lazy_msg)  # Returns lazy proxy
...
def keep_lazy_text(func):
    ...
    Convenience wrapper around keep_lazy(str).
